...
ListElement front(List L);// Returns front element of L. Precondition: length()>0
ListElement back(List L);// Returns back element of L. Precondition: length()>0
ListElement get(List L);// Returns cursor element of L. Pre: length()>0, index()>=0

// Manipulation procedures ----------------------------------------------------
void clear(List L);// Resets L to its original empty state
void set(List L, ListElement x);// Overwrites the cursor element's data with x. Pre: length()>0, index()>=0
...
void prepend(List L, ListElement x);// Insert new element into L. If L is non-empty, insertion takes place before front element
void append(List L, ListElement x); // Insert new element into L. If L is non-empty, insertion takes place after back element
void insertBefore(List L, ListElement x); // Insert new element before cursor. Pre: length()>0, index()>=0
void insertAfter(List L, ListElement x);  // Insert new element after cursor. Pre: length()>0, index()>=0
