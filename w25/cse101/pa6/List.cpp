...
void List::cleanup() {
    int pos = position();  // Tracks the position of `cursor` in the list.
    ...
    do {  // Outer loop iterates through each element in the list.
        ...
        int inner_position = pos + 1;  // Tracks `n`'s position relative to `cursor`.
        ...
        pos++;  // Move to the next element in the outer loop.
